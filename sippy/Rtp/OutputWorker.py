    ...
    def _proc_in(self, now_ns:int, deadline_ns:int):
        ...
        while len(self._enc_buffer) < self._out_psize:
            ...
            if len(audio_enc) > 0:
                ...
                self._pending_chunks.append([len(audio_enc), chunk.nframes])
            else:
                self.update_frm_ctrs(prcsd_inc=chunk.nframes)
        ...
        if len(self._enc_buffer) >= self._out_psize:
            ...
            assert len(packet) == self._out_psize, f'{len(packet)=}, {self._out_psize=}'
