...
def main():
    ...
    parser.add_argument('--rx-wav', default=None, help='optional path to save received audio as WAV')
    ...
    recv_frames = {'count': 0}
    recv_wav_rate = {'value': None}
    recv_wav_data = bytearray()

    def audio_in(chunk: AudioChunk):
        with recv_lock:
            recv_frames['count'] += chunk.nframes
            if args.rx_wav is not None:
                if recv_wav_rate['value'] is None:
                    recv_wav_rate['value'] = chunk.samplerate
                if chunk.samplerate != recv_wav_rate['value']:
                    chunk = AudioChunk(chunk.audio, chunk.samplerate).resample(recv_wav_rate['value'])
                recv_wav_data.extend(chunk.to_bytes())
    ...
    if args.rx_wav is not None:
        with recv_lock:
            out_rate = recv_wav_rate['value']
            out_data = bytes(recv_wav_data)
        if out_rate is None:
            print(f'No received audio, not writing {args.rx_wav}')
        else:
            with wave.open(args.rx_wav, 'wb') as wf:
                wf.setnchannels(1)
                wf.setsampwidth(2)
                wf.setframerate(out_rate)
                wf.writeframes(out_data)
            print(f'Wrote RX WAV: {args.rx_wav}')
